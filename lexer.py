...
def tokenize_expression(expression, line_number, lines):
    for pattern, typ in TOKEN_PATTERNS:
        ...
        if matching is not None: # if a certain pattern has been recognized, then it's not plain text -> we treat it
            after_expression = expression[matching.end():]
            ...
            if typ == "COMMENT":
                ...
                return Token("META", (key.strip(), val.strip()), line_number), after_expression
                ...
                if matching.groups():
                    if len(matching.groups()) > 4:
                        ...
                        return Token(typ, (frame_title, frame_subtitle, frame_options, frame_animations), line_number), after_expression
                else:
                ...
                if matching.groups():
                    if len(matching.groups()) > 2:
                        ...
                        return Token(typ, (code_language, code_options), line_number), after_expression
                else:
                ...
                return Token(typ, pause_animations, line_number), after_expression

            elif typ == "IMAGE":
                if matching.groups()[1]:
                    image_source, args_img = matching.group(1), matching.group(2).split(",")
                    return Token(typ, tuple([image_source, args_img]), line_number ), after_expression
                else:
                    return Token(typ, tuple([matching.group(1), None]), line_number), after_expression

            elif typ == "VIDEO":
                if matching.groups()[1]:
                    video_source, args_vid = matching.group(1), matching.group(2).split(",")
                    return Token(typ, tuple([video_source, args_vid]), line_number), after_expression
                else:
                    return Token(typ, tuple([matching.group(1), None]), line_number), after_expression
                ...
                if matching.groups()[1]:
                    ...
                    return Token(typ, tuple([website_source, args_web]), line_number ), after_expression
                else:
                    return Token(typ, tuple([matching.group(1), None]), line_number), after_expression
                ...
                if matching.groups()[1]:
                    ...
                    return Token(typ, tuple([codefile, args_code]), line_number ), after_expression
                else:
                    return Token(typ, tuple([matching.group(1), None]), line_number), after_expression
                ...
                if matching.groups()[1]:
                    ...
                    return Token(typ, tuple([code_file, language]), line_number ), after_expression
                else:
                    return Token(typ, tuple([matching.group(1), None]), line_number), after_expression
                ...
                if matching.groups():
                    return Token(typ, matching.group(1), line_number), after_expression
                else:
                    return Token(typ, None, line_number), after_expression
